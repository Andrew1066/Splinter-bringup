...
static void read_from_mpl(void) {
	...
	if (hal.report & PRINT_GRAV) {
			if (inv_get_sensor_type_gravity(data, &accuracy,
					(inv_time_t*) &timestamp))
				eMPL_send_data(PACKET_DATA_GRAVITY, data);
	}
...
static int imu_initialized = 0;
void force_imu_self_test() {
	imu_initialized = 0;
}
int imu_main(void)
{ 
	...
	if ( !imu_initialized ){
...
#endif
		hal.dmp_on = 1;
		hal.report = PRINT_ACCEL|PRINT_GYRO|/*PRINT_ROT_MAT|*/PRINT_QUAT|/*PRINT_EULER|*/PRINT_HEADING|PRINT_LINEAR_ACCEL|PRINT_GRAV;
		hal.report = PRINT_ACCEL|PRINT_QUAT|PRINT_GYRO|PRINT_COMPASS|PRINT_GRAV;
		//hal.report = PRINT_ACCEL|PRINT_QUAT;
		hal.rx.cmd = 0;
		hal.next_pedo_ms = 0;
		...
		if ( result )
		{
			_MLPrintLog(0, "", "!!!Failed to load dmp firmware");
		}
		dmp_set_orientation(
				inv_orientation_matrix_to_scalar(gyro_pdata.orientation));
	...
	//_MLPrintLog(0, "", "ts: %d, next_compass: %d, new_gyro: %d\n", timestamp, hal.next_compass_ms, hal.new_gyro);
	if ((timestamp > hal.next_compass_ms) && !hal.lp_accel_mode &&
			hal.new_gyro && (hal.sensors & COMPASS_ON)) {
		hal.next_compass_ms = (timestamp + COMPASS_READ_MS) & 0xffff;
		//SWO_PrintString("new compass\n");
		new_compass = 1;
	}
#endif
