...
void inv_set_gyro_bias(const long *bias, int accuracy)
{
    if (bias != NULL) {
        if (memcmp(inv_data_builder.save.gyro_bias, bias, sizeof(inv_data_builder.save.gyro_bias))) {
            ...
            _MLPrintLog(0, "", "Setting gyro bias t0 %8d %8d %8d\n", bias[0], bias[1], bias[2]);
        }
    }
...
void inv_get_accel_set(long *data, int8_t *accuracy, inv_time_t *timestamp)
{
    if (data != NULL) {
        ...
        memcpy(data + sizeof(sensors.accel.calibrated)/sizeof(sensors.accel.calibrated[0]), sensors.accel.raw_scaled, sizeof(sensors.accel.raw_scaled));
    }
    if (timestamp != NULL) {
        *timestamp = sensors.accel.timestamp;
    }
   ...
   // _MLPrintLog(0, "", "accel: %8d %8d %8d (acc=%d)\n", sensors.accel.raw_scaled[0], sensors.accel.raw_scaled[1], sensors.accel.raw_scaled[2], sensors.accel.accuracy);
}
