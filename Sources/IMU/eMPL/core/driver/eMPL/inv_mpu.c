...
int mpu_send_data(uint8_t addr, uint8_t reg, uint8_t len, uint8_t*data);
int mpu_read_data(uint8_t addr, uint8_t reg, uint8_t len, uint8_t*data);

#define i2c_write   mpu_send_data
#define i2c_read    mpu_read_data
#define delay_ms    OSA_TimeDelay
...
const char* format_float(float val, int buf, int n, int m)
{
	...
	int i = (int)(r * e);
	char sign = val < 0? '-': ' ';
	snprintf(buffer[buf], sizeof(buffer[buf]), "%c%d.%0*d", sign, abs(val), m, i);
	return buffer[buf];
}
...
{
	if ( addr < 2 )
		return SHV_IMU_SendData(addr, reg, len, data);
	//otherwise - compass
	uint8_t bus = addr >> 7;
	addr = addr & 0x7f;
	uint8_t cmd_setup[] = { 0x40, addr };
	uint8_t cmd_write[] = { 0x00, 0x81 };
	uint8_t v;
	int i;

	v = 0x20;
	SHV_IMU_SendData(bus, 0x6a, 1, &v);
	v = data[0];
	SHV_IMU_SendData(bus, 0x63, 1, &v);
	v = 0x40;
	SHV_IMU_SendData(bus, 0x24, 1, &v);
	v = addr;
	SHV_IMU_SendData(bus, 0x25, 1, &v);

	v = 0;
	for(i = 0; i < len; i++)
	{
		v = reg++;
		SHV_IMU_SendData(bus, 0x26, 1, &v);
		v = 0x81;
		SHV_IMU_SendData(bus, 0x27, 1, &v);
	    OSA_TimeDelay(1);
	    v = 0;
		SHV_IMU_SendData(bus, 0x27, 1, &v);
	}
...
{
	if ( addr < 2 )
		return SHV_IMU_ReadData(addr, reg, len, data);
	//otherwise - compass
	uint8_t bus = addr >> 7;
	addr = addr & 0x7f;
	uint8_t cmd_setup[] = { 0x40, addr | 0x80 };
	uint8_t cmd_read[] = { reg, len | 0x80 };
	uint8_t v;
	int i;
	...
	//SHV_IMU_SendData(bus, 0x24, 2, cmd_setup);
	v = 0x40;
	SHV_IMU_SendData(bus, 0x24, 1, &v);
	OSA_TimeDelay(1);
	v = addr | 0x80;
	...
	SHV_IMU_SendData(bus, 0x27, 1, &v);
	OSA_TimeDelay(1);
	SHV_IMU_ReadData(bus, 0x49, len, data);
	return 0;
}
...
static int accel_6500_self_test(long *bias_regular, long *bias_st, int debug)
{
	...
	if(otp_value_zero == 0) {
		...
		for (i = 0; i < 3; i++) {
			...
			if(debug) {
				log_i("Bias_Shift=%s, Bias_Reg=%s, Bias_HWST=%s\r\n",
						FMT74(st_shift_cust[i], 0), FMT74(bias_regular[i], 1),
						FMT74(bias_st[i], 2));
				log_i("OTP value: %s\r\n", FMT74(ct_shift_prod[i], 0));
			}

			st_shift_ratio[i] = st_shift_cust[i] / ct_shift_prod[i] - 1.f;

			if(debug)
				log_i("ratio=%s, threshold=%s\r\n", FMT74(st_shift_ratio[i], 0),
							FMT74(test.max_accel_var, 1));
		...
		if(debug) {
			...
			log_i("Min MG: %s\r\n", FMT74(accel_st_al_min, 0));
			log_i("Max MG: %s\r\n", FMT74(accel_st_al_max, 0));
		}
		...
		for (i = 0; i < 3; i++) {
			...
			if(debug)
				log_i("Bias_shift=%s, st=%s, reg=%s\n", FMT74(st_shift_cust[i]/1.f, 0), FMT74(bias_st[i]/1.f, 1), FMT74(bias_regular[i]/1.f, 2));
...
static int gyro_6500_self_test(long *bias_regular, long *bias_st, int debug)
{
	...
	if(otp_value_zero == 0) {
		...
		for (i = 0; i < 3; i++) {
			...
			if(debug) {
				log_i("Bias_Shift=%s, Bias_Reg=%s, Bias_HWST=%s\r\n",
						FMT74(st_shift_cust[i]/1.f, 0), FMT74(bias_regular[i]/1.f, 1),
						FMT74(bias_st[i]/1.f, 2));
				log_i("OTP value: %s\r\n", FMT74(ct_shift_prod[i]/1.f, 0));
			}

			st_shift_ratio[i] = st_shift_cust[i] / ct_shift_prod[i];

			if(debug)
				log_i("ratio=%s, threshold=%s\r\n", FMT74(st_shift_ratio[i]/1.f, 0),
						FMT74(test.max_gyro_var/1.f, 1));
...
#if 1
	delay_ms(30); //wait 10ms to fill FIFO
...
#define REMOVE_GRAVITY
...
#else
    gyro[0] = gyro[0] * 1000 / packet_count;
    gyro[1] = gyro[1] * 1000 / packet_count;
    gyro[2] = gyro[2] * 1000 / packet_count;
    accel[0] = accel[0] * 1000 / packet_count;
    accel[1] = accel[1] * 1000 / packet_count;
    accel[2] = accel[2] * 1000 / packet_count;
#endif
#else
    ...
    if (accel[2] > 0L)
        ...
        accel[2] += 65536L;
#else
    gyro[0] = gyro[0] * 1000 / s;
    gyro[1] = gyro[1] * 1000 / s;
    gyro[2] = gyro[2] * 1000 / s;
    accel[0] = accel[0] * 1000 / s;
    accel[1] = accel[1] * 1000 / s;
    accel[2] = accel[2] * 1000 / s;
#endif
#endif
/*
    if(debug) {
...
int mpu_run_6500_self_test(long *gyro, long *accel, unsigned char debug)
{
    ...
    _MLPrintLog(0, "", "   accel: %8d %8d %8d\n", accel[0], accel[1], accel[2]);
    _MLPrintLog(0, "", "accel_st: %8d %8d %8d\n", accel_st[0], accel_st[1], accel_st[2]);
    _MLPrintLog(0, "", "   gyro: %8d %8d %8d\n", gyro[0], gyro[1], gyro[2]);
    _MLPrintLog(0, "", "gyro_st: %8d %8d %8d\n", gyro_st[0], gyro_st[1], gyro_st[2]);
...
static int setup_compass(void)
{
    unsigned char data[4], akm_addr = 0xc;
...
//    mpu_set_bypass(1);
    ...
    i2c_read(st.chip_cfg.compass_addr, AKM_REG_WHOAMI, 1, data);
